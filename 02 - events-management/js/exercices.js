...
elementHover.addEventListener("mouseenter", function () {
  elementHover.style.color = "#e74c3c";
});
elementHover.addEventListener("mouseleave", function () {
  elementHover.style.color = "black";
});
...
function validateForm() {
  ...
  for (const input of inputs) {
    if (form[input].value === "") {
      alert(
        `${input.charAt(0).toUpperCase() + input.slice(1)} must be filled out`
      );
      return false;
    }
  }
...
window.addEventListener("scroll", function () {
  ...
  if (scrollY > 3000) {
    // Exemple : Changez la couleur d'arrière-plan à partir de 200 pixels de défilement
